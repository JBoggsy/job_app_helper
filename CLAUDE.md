...
- `backend/models/dspy_example.py` — `DspyExample` model for DSPy training examples (fields: id, module_name, inputs_json, output_json, score, metadata_json, created_at, scored_at)
- `backend/routes/optimize.py` — Optimization blueprint (`optimize_bp` at `/api/optimize`) — trigger BootstrapFewShot optimization, check per-module readiness
...
- `frontend/src/components/OptimizationPanel.jsx` — Slide-out panel for DSPy module optimization; shows per-module readiness (progress bars, scored example counts), "Run Optimization" button, and optimization results
...
| POST | `/api/optimize` | Run BootstrapFewShot optimization on DSPy modules with scored examples |
| GET | `/api/optimize/status` | Check per-module readiness (scored/unscored counts, optimization state) |
...
- **DSPy optimization (fixed_pipeline only):** The fixed pipeline agent uses DSPy modules for structured-output micro-agents. Training examples are collected passively from pipeline runs and scored from user actions (tracker adds, fit edits). `POST /api/optimize` runs BootstrapFewShot; compiled modules persist in `dspy_modules/` and auto-load on startup. See `docs/DSPy_OPTIMIZATION.md` for the full guide.
